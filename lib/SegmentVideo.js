...
SegmentVideo.prototype = assign({}, SegmentTimeline.prototype, {
  ...
  computeCurrentTime: function (relativeTime) {
    var opts = this.opts;
    return ((opts.position + relativeTime * opts.playbackRate) / 1000) % this.videoDuration;
  },
  ...
  render: function (t, interval) {
    ...
    var now = Date.now();
    var videoPlaying = !video.paused;
    var diaporamaPlaying = (now - this._lastRender||0) < 100;
    var dirtyPlayState = videoPlaying !== diaporamaPlaying;
    this._lastRender = now;

    if (Math.abs((currentTime - video.currentTime + dur) % dur) > 0.3) {
