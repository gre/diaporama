  ...
  */
  ...
  tmplSlider = function(o) {
    ...
    slider.find('.slide-images').append($.map(o.slides, function(slide) {
      return $('<div class="slide-image">' + (slide.link ? '<a href="' + slide.link + '" target="_blank">' : '') + '<img src="' + slide.src + '">' + (slide.name ? '<span class="caption">' + slide.name + '</span>' : '') + (slide.description ? '<span class="description">' + slide.description + '</span>' : '') + (slide.link ? '</a>' : '') + '</div>')[0];
    }));
    
    slider.find('.slide-pager').append($.map(o.slides, function(slide, i) {
      return $('<a href="javascript:;" title="' + (slide.name ? '<span class="caption">' + slide.name + '</span>' : '') + '">' + (i + 1) + '</a>')[0];
    }));
    return slider;
  };
    ...
    Slider.prototype._bind = function() {
      ...
      if (!this.timeout) {
        loop_ = __bind(function() {
          ...
          return this.timeout =  (this.duration) ? {} : setTimeout(loop_, this.duration);
        }, this);
        this.timeout = (this.duration) ? {} : setTimeout(loop_, this.duration);
      }
      return this;
    };
