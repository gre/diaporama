(function() {
  var Slider, SliderWithCanvas, YantsTransitionFunctions, currentTime, mod, requestAnimationFrame, simpleTransitionFunction, tmplSlider, tmplSliderWithCanvas;
  ...
  Slider = (function() {
    ...
    Slider.prototype.setSize = function(w, h) {
      ...
      if (this.node) {
        ...
        this.node.find(".slide-images, .slider-image").height(h);
      }
      return this;
    };
  ...
  simpleTransitionFunction = function(clipFunction) {
    return {
      render: function(self, from, to, progress) {
  ...
  YantsTransitionFunctions = {
    clock: simpleTransitionFunction(function(ctx, w, h, p) {
      ctx.moveTo(w / 2, h / 2);
      return ctx.arc(w / 2, h / 2, Math.max(w, h), 0, Math.PI * 2 * p, false);
    }),
    circle: simpleTransitionFunction(function(ctx, w, h, p) {
      ctx.moveTo(w / 2, h / 2);
      return ctx.arc(w / 2, h / 2, 0.6 * p * Math.max(w, h), 0, Math.PI * 2, false);
    }),
    verticalOpen: simpleTransitionFunction(function(ctx, w, h, p) {
      return ctx.rect((1 - p) * w / 2, 0, w * p, h);
    }),
    horizontalOpen: simpleTransitionFunction(function(ctx, w, h, p) {
  ...
  SliderWithCanvas = (function() {
    ...
    SliderWithCanvas.prototype.transitionFunction = YantsTransitionFunctions.clock;
    ...
    SliderWithCanvas.prototype.render = function() {
      var now, progress;
      now = currentTime();
      if (now >= this.transitionStart) {
