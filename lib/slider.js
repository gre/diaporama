...
(function() {
  ...
  SliderTransitionFunctions = {
    ...
    verticalOpen: {
      render: SliderUtils.clippedTransition(function(ctx, w, h, p) {
        var h1, h2, hi, nbSpike, pw, spikeh, spikel, spiker, spikew, xl, xr, _i, _j, _results;
        nbSpike = 8;
        spikeh = h / (2 * nbSpike);
        spikew = spikeh;
        pw = p * w / 2;
        ...
        spiker = xr + spikew;
        ctx.moveTo(xl, 0);
        for (hi = _i = 0; 0 <= nbSpike ? _i <= nbSpike : _i >= nbSpike; hi = 0 <= nbSpike ? ++_i : --_i) {
          h1 = (2 * hi) * spikeh;
          h2 = h1 + spikeh;
        ...
        ctx.lineTo(spiker, h);
        _results = [];
        for (hi = _j = nbSpike; nbSpike <= 0 ? _j <= 0 : _j >= 0; hi = nbSpike <= 0 ? ++_j : --_j) {
          h1 = (2 * hi) * spikeh;
          h2 = h1 - spikeh;
          ctx.lineTo(xr, h1);
    ...
    horizontalSunblind: {
      render: SliderUtils.clippedTransition(function(ctx, w, h, p) {
        var blind, blindHeight, blinds, _i, _results;
        p = 1 - (1 - p) * (1 - p);
        blinds = 6;
        blindHeight = h / blinds;
        _results = [];
        for (blind = _i = 0; 0 <= blinds ? _i <= blinds : _i >= blinds; blind = 0 <= blinds ? ++_i : --_i) {
    ...
    verticalSunblind: {
      render: SliderUtils.clippedTransition(function(ctx, w, h, p) {
        var blind, blindWidth, blinds, prog, _i, _results;
        p = 1 - (1 - p) * (1 - p);
        blinds = 10;
        blindWidth = w / blinds;
        _results = [];
    ...
    circles: {
      render: SliderUtils.clippedTransition(function(ctx, w, h, p) {
        var circleH, circleW, circlesX, circlesY, cx, cy, maxRad, maxWH, r, x, y, _i, _results;
        circlesY = 6;
        ...
        _results = [];
        for (x = _i = 0; 0 <= circlesX ? _i <= circlesX : _i >= circlesX; x = 0 <= circlesX ? ++_i : --_i) {
          _results.push((function() {
            var _j, _results1;
            _results1 = [];
            for (y = _j = 0; 0 <= circlesY ? _j <= circlesY : _j >= circlesY; y = 0 <= circlesY ? ++_j : --_j) {
              cx = (x + 0.5) * circleW;
              cy = (y + 0.5) * circleH;
              r = Math.max(0, Math.min(2 * p - cx / w, 1)) * maxRad;
              ctx.moveTo(cx, cy);
              _results1.push(ctx.arc(cx, cy, r, 0, Math.PI * 2, false));
            }
            return _results1;
          })());
        }
    ...
    squares: {
      render: SliderUtils.clippedTransition(function(ctx, w, h, p) {
        var blindHeight, blindWidth, blindsX, blindsY, prog, rh, rw, sx, sy, x, y, _i, _results;
        p = 1 - (1 - p) * (1 - p);
        blindsY = 5;
        ...
        _results = [];
        for (x = _i = 0; 0 <= blindsX ? _i <= blindsX : _i >= blindsX; x = 0 <= blindsX ? ++_i : --_i) {
          _results.push((function() {
            var _j, _results1;
            _results1 = [];
            for (y = _j = 0; 0 <= blindsY ? _j <= blindsY : _j >= blindsY; y = 0 <= blindsY ? ++_j : --_j) {
              sx = blindWidth * x;
              sy = blindHeight * y;
              prog = Math.max(0, Math.min(3 * p - sx / w - sy / h, 1));
              rw = blindWidth * prog;
              rh = blindHeight * prog;
              _results1.push(ctx.rect(sx - rw / 2, sy - rh / 2, rw, rh));
            }
            return _results1;
          })());
        }
  ...
  Slider = (function() {
    ...
    Slider.prototype.slide = function(num) {
      ...
      this.current = num;
      if (this.onSlideCallbacks != null) {
        _ref = this.onSlideCallbacks;
    ...
    Slider.prototype.stop = function() {
      var cb, _i, _len, _ref;
      this._unbind();
      if (this.onStopCallbacks != null) {
        _ref = this.onStopCallbacks;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          cb = _ref[_i];
          cb();
        }
    ...
    Slider.prototype._bind = function() {
      ...
      if (!this.timeout) {
        loop_ = function() {
          if (now() - _this.lastHumanNav > 2000) {
            _this.next();
          }
    ...
    Slider.prototype.onStart = function(callback) {
      this.onStartCallbacks || (this.onStartCallbacks = []);
      this.onStartCallbacks.push(callback);
      return this;
    };

    Slider.prototype.onStop = function(callback) {
      this.onStopCallbacks || (this.onStopCallbacks = []);
      this.onStopCallbacks.push(callback);
      return this;
    };

    Slider.prototype.onSlide = function(callback) {
      this.onSlideCallbacks || (this.onSlideCallbacks = []);
  ...
  SliderWithCanvas = (function(_super) {

    __extends(SliderWithCanvas, _super);

    function SliderWithCanvas() {
      return SliderWithCanvas.__super__.constructor.apply(this, arguments);
    }
