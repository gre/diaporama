...
Diaporama.prototype = assign({}, EventEmitter.prototype, {
  ...
  _loadMedia: function () {
    ...
    this._data.timeline.forEach(function (item) {
      if (item.video)
        addSrc(item.video, MediaTypes.VIDEO);
      else if (item.image)
        addSrc(item.image, MediaTypes.IMAGE);
      else if (item.slide2d)
        forEachSlide2dImage(item.slide2d.draws, function (src) {
          addSrc(src, MediaTypes.IMAGE);
        });
    });
    ...
    function timeZeroReady () {
      var segments = self._segments;
      var rendering = self._rendering;
      var seglen = segments.length;
      for (var i=0; i<seglen; ++i) {
        var segment = segments[i];
        if (segment[0].timeInside(0)) {
    ...
    function watch () {
      Channels.SLIDE2D.forEach(function (channel) {
        rendering.getChannelContext(channel).setImgsCache(media.resources);
      });
      ...
      if (ready.length === Object.keys(toload).length) {
        ...
        if (_canplay && !_canplaythrough) {
          ...
          if (0.8 * ready.length / Object.keys(toload).length > elapsed / totalDuration) {
            canplaythrough();
          }
        }
      }
    ...
    function onLoad (src) {
      ...
      self.emit("progress", ready.length / Object.keys(toload).length);
      ready.push(src);
      watch();
    }
    ...
    for (var key in toload) {
      media.load(this._resolve(toload[key]), types[key]);
    }
  ...
  _computeTimelineSegments: function () {
    ...
    while ( (segment = this._segments.pop()) ) {
    ...
    for (var i = 0; i < tlength; i++) {
      ...
      if (item.video) {
        item.video = this._resolve(item.video);
        if (i===0) {
          extraLoopSegment = new this._rendering.SegmentKenBurns(Channels.KENBURNS[channelIndex], item, this);
        }
        segment = new this._rendering.SegmentKenBurns(Channels.KENBURNS[channelIndex], item, this);
      }
      else if (item.image) {
        item.image = this._resolve(item.image);
        if (i===0) {
          var staticItem = assign({}, item);
          staticItem.kenburns = assign({}, staticItem.kenburns);
          staticItem.kenburns.to = staticItem.kenburns.from;
          extraLoopSegment = new this._rendering.SegmentKenBurns(Channels.KENBURNS[channelIndex], staticItem, this);
        }
        segment = new this._rendering.SegmentKenBurns(Channels.KENBURNS[channelIndex], item, this);
        ...
        if (i===0) extraLoopSegment = new this._rendering.SegmentSlide2d(Channels.SLIDE2D[channelIndex], item);
  ...
  _render: function () {
    ...
    if (segments.length===0) {
      rendering.render(0, []);
      this.emit("render", 0, RENDER_NO_SEGMENTS);
    ...
    var state = runningSegments.length > 0 ?
      (notReady ? RENDER_NOT_READY : RENDER_READY) :
      RENDER_NO_SEGMENTS;
  ...
  _resolve: function (descr) {
    var data = this.data;
    return typeof descr === "string" &&
      data &&
      data.resources &&
...
Diaporama.RENDER_NO_SEGMENTS = RENDER_NO_SEGMENTS;
Diaporama.RENDER_NOT_READY = RENDER_NOT_READY;
Diaporama.RENDER_READY = RENDER_READY;
