...
Diaporama.prototype = assign({}, EventEmitter.prototype, {
  ...
  _computeTimelineSegments: function () {
    ...
    for (var i = 0; i < diaporama.timeline.length; i++) {
      ...
      segments.push([
        new TimeInterval(t - lastTransitionDuration, t + item.duration + tnextDuration),
        new SegmentKenBurns(kenburnsChannels[0], item, i)
      ]);
      ...
      if (tnextDuration) {
        ...
        if (j !== 0 || this._loop) {
          segments.push([
            new TimeInterval(t, t + tnextDuration),
            new SegmentTransition(Channels.TRANSITION, kenburnsChannels, tnext, i, j, diaporama.timeline[i].image, diaporama.timeline[j].image)
          ]);
        }
        t += tnextDuration;
      }
