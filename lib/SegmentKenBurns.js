...
SegmentKenBurns.prototype = assign({}, SegmentTimeline.prototype, {
  ...
  _load: function (ctx) {
    ...
    if (this.isFirstInstance) {
      var res = ctx.media.get(data.image || data.video);
      if (res) {
        if (data.image) this.image = res;
        else if (data.video) {
          if (res.video) {
            this.video = res.video;
            this._computeVideoMeta();
          }
          else if (res.image) this.image = res.image;
        }
        this._loaded = true;
      }
      return;
    ...
    this._loaded = true;
    if (data.video) {
      this._res = loaders.video(data.video, function (result) {
        if (result.video) {
          self.video = result.video;
          self._computeVideoMeta();
        }
      ...
      this._res = loaders.image(data.image, function (img) {
        self.image = img;
      });
    }
  },
