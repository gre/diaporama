  ...
  render: function (t, interval) {
    ...
    if (video) {
      ...
      if (!this.awaiting) {
        ...
        if (this.enteredNoRender && !diaporamaPaused) video.play();
      }

      // If the video.paused is STILL not in sync, force the currentTime (e.g: iPhone / iPad doesn't allow to .play())
